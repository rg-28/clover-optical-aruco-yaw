...
count = 1
while(count <= 3):
    navigate_wait(x=0.0, y=0.0, z=count, speed=0.5, frame_id=aruco_id)
    #Each time an increase of 1 m than the current position in z-axis is sent to the drone with an upward velocity of 0.5m/s.
    rospy.sleep(5.0)
    ...
    rospy.sleep(12.56) 
    # The delay of 12.56 seconds is calculated by dividing the required yaw angle (360 degrees)
    # in radians (6.283 radians) by the current yaw rate (0.5 rad/s).
    telemetry = get_telemetry(frame_id=aruco_id)
